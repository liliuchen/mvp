...
//					dos.flush();
			...
			int len = 0;
			int curLen = 0;
			while ((len = is.read(bytes)) != -1&&!interceptFlag) {
				curLen += len;
...
//				Gson sGson=new Gson();
//				BaseMode b=sGson.fromJson(result, BaseMode.class);
	...
	{
		if(null!=onUploadProcessListener)onUploadProcessListener.onUploadDone(responseCode, responseMessage);
	}
